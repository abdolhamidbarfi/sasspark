 ...
 */
const OptionSection = ({ slice , index }: OptionSectionProps): JSX.Element => {
    return (
        <section className="w-full bg-gray-100"
            data-slice-type={slice.slice_type}
            data-slice-variation={slice.variation}
        >
            <div key={index} className=" container mx-auto px-5 transition-all duration-1000 py-10">
                ...
                <ul className="flex md:justify-around md:items-center max-md:flex-col px-5">
                    {
                        slice.items.map(
                            (item , index) => (
                                <li key={index} className="basis-1/4 mb-5">
